...
function showNotification(chatId, messageContent) {
  try {
    const currentState = window.state || state; // 兼容不同作用域的 state 存取
    if (!currentState || !currentState.chats || !currentState.chats[chatId]) return;

    const chat = currentState.chats[chatId];
    const disableInternal = currentState.globalSettings?.systemNotification?.disableInternalNotification || false;
    ...
    if (!disableInternal) {
      ...
      if (bar) {
        const avatarSrc = chat.settings?.aiAvatar || chat.settings?.groupAvatar || window.defaultAvatar || 'assets/default-avatar.png';
        ...
        if (contentEl) {
          contentEl.textContent = `${chat.name}: ${messageContent}`;
        }
      ...
      if (typeof window.updateBackButtonUnreadCount === 'function') window.updateBackButtonUnreadCount();
    }

    if (currentState.globalSettings?.systemNotification?.enabled && typeof window.handleSystemNotification === 'function') {
      window.handleSystemNotification(chatId, messageContent);
    }
  } catch (error) {
    console.error('[Entropy: showNotification Error]:', error);
  }
}
